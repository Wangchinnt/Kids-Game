...
public class TwoNumberPlusGame : MiniGame
{
    ...
    {
        gameName = "TwoNumberPlusGame";
        isCompleted = false;
        Debug.Log("TwoNumberPlusGame initialized");
    ...
    private int GenerateTwoDigitNonCarry()
    {
        int tens = Random.Range(1, 10);  // Hàng chục từ 1 đến 9
        int units = Random.Range(0, 10 - tens); // Hàng đơn vị đảm bảo không nhớ khi cộng
        return tens * 10 + units;
    }
    private bool IsNonCarry(int a, int b)
    {
        return ((a % 10) + (b % 10) < 10);  // Kiểm tra hàng đơn vị có bị nhớ không
    }
    ...
    private void GenerateOperations( )
    {
        foreach (var answerArea in answerAreas)
        {
            ...
            int num1, num2, answerValue;

            do
            {
                num1 = GenerateTwoDigitNonCarry();
                num2 = GenerateTwoDigitNonCarry();
                answerValue = num1 + num2;
            } while (answerValue > 100 || !IsNonCarry(num1, num2)); // Đảm bảo tổng hợp lệ
            ...
            switch (format)
            {
                ...
                case 1: 
                    // Trường hợp a + ? = b (tìm num2)
                    do
                    {
                        num2 = answerValue - num1;
                    } while (num2 < 10 || num2 >= 100 || !IsNonCarry(num1, num2)); // Kiểm tra tính hợp lệ
                    addends[1] = num2;
                    break;

                case 2: 
                    // Trường hợp ? + a = b (tìm num1)
                    do
                    {
                        num1 = answerValue - num2;
                    } while (num1 < 10 || num1 >= 100 || !IsNonCarry(num1, num2)); // Kiểm tra tính hợp lệ
                    addends[0] = num1;
                    break;
            }
