...
public class PractiseMinusGame : MiniGame
{
    ...
    private void GenerateOperations()
    {
        foreach (var answerArea in answerAreas)
        {
            ...
            while (addends[0] - addends[1] <= 0)
            {
                addends[0] = Random.Range(operationRange[0], operationRange[1]);
                addends[1] = Random.Range(operationRange[0], operationRange[1]);
            }
            int answerValue = addends[0] - addends[1];
    ...
    private void SufferAnswers()
    {
        ...
        sequence.OnComplete(() =>
        {   
            DOVirtual.DelayedCall(0.5f, () => // Delay 5s
            {   
                ...
                for (int i = 0; i < answerButtons.Count; i++)
                {
                    ...
                    answerButtons[i].transform.DOScale(Vector3.one * 1.5f, 0.5f)
                        ...
                        .OnComplete(() =>
                        {
                            // After appearing, activate the shake effect
